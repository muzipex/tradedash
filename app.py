...
import threading
import numpy as np
from appium_automation import MT5MobileAutomation  # Import the Appium automation class
...
app = Flask(__name__)
CORS(app, resources={
    r"/api/*": {"origins": "https://your-frontend-url.com"},  # Update this URL
    r"/ws/*": {"origins": "https://your-frontend-url.com"}    # Update this URL
})
...
def login():
    ...
    try:
        ...
        if not user:
            user = User(account_id=data['accountId'], server=data['server'])
            db.session.add(user)
        else:
        ...
        # Use Appium to automate the login process
        automation = MT5MobileAutomation(
            device_name="Android Emulator",  # Default device name for emulator
            platform_version="11.0",  # Default platform version
            app_package=os.getenv("APP_PACKAGE", "com.metaquotes.metatrader5"),
            app_activity=os.getenv("APP_ACTIVITY", ".MainActivity"),
            appium_server_url="http://localhost:4723/wd/hub"  # Update with your Appium server URL and port
        )
        success = automation.login(data['accountId'], data['password'], data['server'])

        if success:
            # Create and store bot instance
            bot_key = f"{data['accountId']}_{data['server']}"
            bot = MT5SMCBot(data['accountId'], data['password'], data['server'])
            bot.connected = True  # Set connected to True as Appium handled the login
            bots[bot_key] = bot
